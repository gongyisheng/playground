"""Sweep int4 per-tensor quant error to find where it breaks vs stays safe.

Reuses the quantizer from int4_quant.py. Goal: show error as a function of the
distribution's dynamic range (driven here by outliers), and confirm that the
single predictor SNR ≈ 12 · 15² · (std/range)² tracks measured error.
"""

import torch

from int4_quant import quantize, dequantize
...
def measure(x: torch.Tensor) -> dict:
    ...
    return {
        ...
        "used_levels": q.unique().numel(),     # distinct int4 codes actually used, of 16
    }
...
if __name__ == "__main__":
    ...
    print(f"{'outlier×std':>11} {'scale':>10} {'used/16':>9} "
