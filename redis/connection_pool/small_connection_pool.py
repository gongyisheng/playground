...
async def main(pool_size, coro_num, io_time, round):
    pool = init(pool_size)
    tasks = [asyncio.create_task(io_work(pool, io_time, round)) for _ in range(coro_num)]
...
if __name__ == '__main__':
    ...
    loop.run_until_complete(main(pool_size=pool_size, coro_num=coro_num, io_time=io_time, round=round_num))
