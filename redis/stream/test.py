...
async def producer(num, task_id, batch_size=10):
    ...
    print(
        f"Data size: {len(data)/1024} KB, batch_data_size={len(data)*batch_size/1024} KB"
    )
    ...
    print(
        f"[{task_id}] Added {num} messages with 10KB data each to Redis stream in {elapsed_time} seconds"
    )
...
async def consumer(num, task_id, batch_size=10):
    ...
    await r.xclaim(
        f"mystream_{task_id}", group_name, consumer_name, min_idle_time, ["0-0"]
    )
    ...
    # Print benchmark results
    print(
        f"[{task_id}] Read {num} messages with 10KB data from Redis stream in {elapsed_time} seconds"
    )
...
async def main(num, concurrency, batch_size=10):
    ...
    tasks = []
    tasks += [
        asyncio.create_task(producer(num, i, batch_size)) for i in range(concurrency)
    ]
    tasks += [
        asyncio.create_task(consumer(num, i, batch_size)) for i in range(concurrency)
    ]
    await asyncio.gather(*tasks)
